...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList)
{
    ...
    for (const auto &t : TriangleList)
    {
        ...
        // viewspace_pos是相机空间的顶点坐标：在光栅化器中需要使用相机空间的顶点坐标对三角形内每个像素点坐标进行插值。插值出来的坐标会送入shader中，并在之后的phong_fragment_shader等过程中作为shader point使用。
        // 为什么shader point要使用相机空间的顶点坐标，不用经过了mvp矩阵的屏幕空间坐标？ -> 原因是在光照模型中需要用到法向量，法向量只能在相机空间中计算。因为P矩阵和透视除法会改变原来的三角形的z值，经过P矩阵和透视除法后的NDC坐标系中，保留z值只是为了做深度判断，这个z值已经不是原来三角形的z值了，无法在裁剪空间和NDC空间求出法向量。因此因为法向量在相机空间，shader point也就需要在相机空间，这样才能使用光照模型。
